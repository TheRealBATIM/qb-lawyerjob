...
-- Events
RegisterNetEvent('lawyer:client:ActiveCamera', function(cameraId)
...
RegisterNetEvent('lawyer:client:DisableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = false
    end
end)

RegisterNetEvent('lawyer:client:EnableAllCameras', function()
    for k, _ in pairs(Config.SecurityCameras.cameras) do
        Config.SecurityCameras.cameras[k].isOnline = true
    end
end)

RegisterNetEvent('lawyer:client:SetCamera', function(key, isOnline)
    ...
    elseif type(key) == 'number' then
        ...
        error('lawyer:client:SetCamera did not receive the right type of key\nreceived type: ' .. type(key) .. '\nreceived value: ' .. key)
    end
end)
