...
local function UpdateBlips()
    ...
    for _, v in pairs(players) do
        if v and (v.PlayerData.job.name == "lawyer" or v.PlayerData.job.name == "ambulance" or v.Player.job.name == "police" or v.PlayerData.job.name == "mechanic") and v.PlayerData.job.onduty then
    ...
    TriggerClientEvent("lawyer:client:UpdateBlips", -1, dutyPlayers)
end
...
local function GetCurrentCops()
    ...
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "lawyer" and v.PlayerData.job.onduty then
            amount += 1
        end
    end
    return amount
...
QBCore.Commands.Add("spikestrip", Lang:t("commands.place_spike"), {}, false, function(source)
    ...
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent('lawyer:client:SpawnSpikeStrip', src)
    end
end)
...
QBCore.Commands.Add("grantlicense", Lang:t("commands.license_grant"), {{name = "id", help = Lang:t('info.player_id')}, {name = "license", help = Lang:t('info.license_type')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.grade.level >= Config.LicenseRank then
...
QBCore.Commands.Add("pobject", Lang:t("commands.place_object"), {{name = "type",help = Lang:t("info.poobject_object")}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        if type == "cone" then
            TriggerClientEvent("lawyer:client:spawnCone", src)
        elseif type == "barrier" then
            TriggerClientEvent("lawyer:client:spawnBarrier", src)
        elseif type == "roadsign" then
            TriggerClientEvent("lawyer:client:spawnRoadSign", src)
        elseif type == "tent" then
            TriggerClientEvent("lawyer:client:spawnTent", src)
        elseif type == "light" then
            TriggerClientEvent("lawyer:client:spawnLight", src)
        elseif type == "delete" then
            TriggerClientEvent("lawyer:client:deleteObject", src)
        end
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cuff", Lang:t("commands.cuff_player"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:CuffPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("escort", Lang:t("commands.escort"), {}, false, function(source)
    local src = source
    TriggerClientEvent("lawyer:client:EscortPlayer", src)
end)
...
QBCore.Commands.Add("jail", Lang:t("commands.jail_player"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:JailPlayer", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("seizecash", Lang:t("commands.seizecash"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:SeizeCash", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("sc", Lang:t("commands.softcuff"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:CuffPlayerSoft", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("cam", Lang:t("commands.camera"), {{name = "camid", help = Lang:t('info.camera_id')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:ActiveCamera", src, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:ImpoundVehicle", src, false, tonumber(args[1]))
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("impound", Lang:t("commands.impound"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:ImpoundVehicle", src, true)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
    ...
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        ...
        if OtherPlayer then
            if OtherPlayer.PlayerData.job.name == "tow" then
                OtherPlayer.Functions.AddMoney("bank", 500, "lawyer-tow-paid")
...
QBCore.Commands.Add("paylawyer", Lang:t("commands.paylawyer"), {{name = "id",help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyere" or Player.PlayerData.job.name == "judge" then
        local playerId = tonumber(args[1])
        local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
        if not OtherPlayer then return end
        if OtherPlayer.PlayerData.job.name == "lawyer" then
            OtherPlayer.Functions.AddMoney("bank", 500, "lawyer-lawyer-paid")
            TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("success.tow_paid"), 'success')
...
QBCore.Commands.Add('fine', Lang:t("commands.fine"), {{name = 'id', help = Lang:t('info.player_id')}, {name = 'amount', help = Lang:t('info.amount')}}, false, function(source, args)
    ...
    if biller.PlayerData.job.name == "lawyer" then
        if billed ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', source, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("anklet", Lang:t("commands.anklet"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:CheckDistance", src)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("ankletlocation", Lang:t("commands.ankletlocation"), {{name = "cid", help = Lang:t('info.citizen_id')}}, true, function(source, args)
    ...
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        ...
        if Target.PlayerData.metadata["tracker"] then
            TriggerClientEvent("lawyer:client:SendTrackerLocation", Target.PlayerData.source, src)
        else
...
QBCore.Commands.Add("takedrivinglicense", Lang:t("commands.drivinglicense"), {}, false, function(source)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.name == "lawyer" and Player.PlayerData.job.onduty then
        TriggerClientEvent("lawyer:client:SeizeDriverLicense", source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.on_duty_lawyer_only"), 'error')
    end
end)
...
QBCore.Commands.Add("takedna", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    ...
    if not OtherPlayer or Player.PlayerData.job.name ~= "lawyer" or not Player.PlayerData.job.onduty then return end
...
RegisterNetEvent('lawyer:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("lawyer:client:TrackerMessage", requestId, msg, coords)
...
QBCore.Functions.CreateUseableItem("moneybag", function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player or not Player.Functions.GetItemByName("moneybag") or not item.info or item.info == "" or Player.PlayerData.job.name == "lawyer" or not Player.Functions.RemoveItem("moneybag", 1, item.slot) then return end
    Player.Functions.AddMoney("cash", tonumber(item.info.cash), "used-moneybag")
end)

-- Callbacks
QBCore.Functions.CreateCallback('lawyer:server:isPlayerDead', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    cb(Player.PlayerData.metadata["isdead"])
end)

QBCore.Functions.CreateCallback('lawyer:GetPlayerStatus', function(_, cb, playerId)
    local Player = QBCore.Functions.GetPlayer(playerId)
    local statList = {}
    if Player then
...
QBCore.Functions.CreateCallback('lawyer:IsSilencedWeapon', function(source, cb, weapon)
...
QBCore.Functions.CreateCallback('lawyer:GetDutyPlayers', function(_, cb)
    local dutyPlayers = {}
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "lawyer" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('lawyer:GetImpoundedVehicles', function(_, cb)
    local vehicles = {}
...
QBCore.Functions.CreateCallback('lawyer:IsPlateFlagged', function(_, cb, plate)
    local retval = false
...
QBCore.Functions.CreateCallback('lawyer:GetCops', function(_, cb)
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "lawyer" and v.PlayerData.job.onduty then
...
QBCore.Functions.CreateCallback('lawyer:server:IsPoliceForcePresent', function(_, cb)
    local retval = false
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == "lawyer" and v.PlayerData.job.grade.level >= 2 then
            retval = true
            break
        end
    end
...
AddEventHandler('onResourceStart', function(resourceName)
    if resourceName == GetCurrentResourceName() then
        CreateThread(function()
            MySQL.query("DELETE FROM stashitems WHERE stash = 'lawyertrash'")
        end)
    end
end)

RegisterNetEvent('lawyer:server:policeAlert', function(text)
    local src = source
    ...
    for _, v in pairs(players) do
        if v and v.PlayerData.job.name == 'lawyer' and v.PlayerData.job.onduty then
            ...
            TriggerClientEvent('lawyer:client:policeAlert', v.PlayerData.source, coords, text)
        end
    end
end)

RegisterNetEvent('lawyer:server:TakeOutImpound', function(plate, garage)
    local src = source
...
RegisterNetEvent('lawyer:server:CuffPlayer', function(playerId, isSoftcuff)
    local src = source
    local playerPed = GetPlayerPed(src)
    ...
    if not Player or not CuffedPlayer or (not Player.Functions.GetItemByName("handcuffs") and Player.PlayerData.job.name ~= "lawyer") then return end

    TriggerClientEvent("lawyer:client:GetCuffed", CuffedPlayer.PlayerData.source, Player.PlayerData.source, isSoftcuff)
end)

RegisterNetEvent('lawyer:server:EscortPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if (Player.PlayerData.job.name == "lawyer" or Player.PlayerData.job.name == "ambulance") or (EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"]) then
        TriggerClientEvent("lawyer:client:GetEscorted", EscortPlayer.PlayerData.source, Player.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('lawyer:server:KidnapPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
        TriggerClientEvent("lawyer:client:GetKidnappedTarget", EscortPlayer.PlayerData.source, Player.PlayerData.source)
        TriggerClientEvent("lawyer:client:GetKidnappedDragger", Player.PlayerData.source, EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('lawyer:server:SetPlayerOutVehicle', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
        TriggerClientEvent("lawyer:client:SetOutVehicle", EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('lawyer:server:PutPlayerInVehicle', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["isdead"] then
        TriggerClientEvent("lawyer:client:PutInVehicle", EscortPlayer.PlayerData.source)
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.not_cuffed_dead"), 'error')
    end
end)

RegisterNetEvent('lawyer:server:BillPlayer', function(playerId, price)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(playerId)
    if not Player or not OtherPlayer or Player.PlayerData.job.name ~= "lawyer" then return end

    OtherPlayer.Functions.RemoveMoney("bank", price, "paid-bills")
    exports['qb-management']:AddMoney("lawyer", price)
    TriggerClientEvent('QBCore:Notify', OtherPlayer.PlayerData.source, Lang:t("info.fine_received", {fine = price}))
end)

RegisterNetEvent('lawyer:server:JailPlayer', function(playerId, time)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    TriggerClientEvent("lawyer:client:SendToJail", OtherPlayer.PlayerData.source, time)
    TriggerClientEvent('QBCore:Notify', src, Lang:t("info.sent_jail_for", {time = time}))
end)

RegisterNetEvent('lawyer:server:SetHandcuffStatus', function(isHandcuffed)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player then
...
RegisterNetEvent('lawyer:server:SearchPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
...
RegisterNetEvent('lawyer:server:SeizeCash', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    SearchedPlayer.Functions.RemoveMoney("cash", moneyAmount, "lawyer-cash-seized")
...
RegisterNetEvent('lawyer:server:SeizeDriverLicense', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
...
RegisterNetEvent('lawyer:server:RobPlayer', function(playerId)
    local src = source
    local playerPed = GetPlayerPed(src)
    local targetPed = GetPlayerPed(playerId)
    local playerCoords = GetEntityCoords(playerPed)
    ...
    Player.Functions.AddMoney("cash", money, "lawyer-player-robbed")
    SearchedPlayer.Functions.RemoveMoney("cash", money, "lawyer-player-robbed")
...
RegisterNetEvent('lawyer:server:UpdateBlips', function()
    -- KEEP FOR REF BUT NOT NEEDED ANYMORE.
end)

RegisterNetEvent('lawyer:server:spawnObject', function(type)
    local src = source
    local objectId = CreateObjectId()
    Objects[objectId] = type
    TriggerClientEvent("lawyer:client:spawnObject", src, objectId, type, src)
end)

RegisterNetEvent('lawyer:server:deleteObject', function(objectId)
    TriggerClientEvent('lawyer:client:removeObject', -1, objectId)
end)

RegisterNetEvent('lawyer:server:Impound', function(plate, fullImpound, price, body, engine, fuel)
    local src = source
...
RegisterNetEvent('lawyer:server:UpdateCurrentCops', function()
    local amount = 0
    local players = QBCore.Functions.GetQBPlayers()
    ...
    TriggerClientEvent("lawyer:SetCopCount", -1, amount)
    updatingCops = false
end)
...
RegisterNetEvent('lawyer:server:showFingerprint', function(playerId)
    local src = source
    TriggerClientEvent('lawyer:client:showFingerprint', playerId, src)
    TriggerClientEvent('lawyer:client:showFingerprint', src, playerId)
end)

RegisterNetEvent('lawyer:server:showFingerprintId', function(sessionId)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local fid = Player.PlayerData.metadata["fingerprint"]
    TriggerClientEvent('lawyer:client:showFingerprintId', sessionId, fid)
    TriggerClientEvent('lawyer:client:showFingerprintId', src, fid)
end)

RegisterNetEvent('lawyer:server:SetTracker', function(targetId)
    local src = source
    local playerPed = GetPlayerPed(src)
    ...
    if TrackerMeta then
        ...
        TriggerClientEvent('lawyer:client:SetTracker', targetId, false)
    else
        ...
        TriggerClientEvent('lawyer:client:SetTracker', targetId, true)
    end
end)

RegisterNetEvent('lawyer:server:SyncSpikes', function(table)
    TriggerClientEvent('lawyer:client:SyncSpikes', -1, table)
end)
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent("lawyer:SetCopCount", -1, curCops)
    end
end)
